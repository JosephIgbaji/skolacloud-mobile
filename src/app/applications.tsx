...
export default function ApplicationsScreen() {
  ...
  const { data: appsList = [], isLoading, isError, refetch } = useQuery({
    queryKey: ['admin-applications-list'],
    queryFn: async () => {
      const res = await apiClient.get('/applications').catch(() => ({ data: [] }));
      const raw = res.data;
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <ThemedText style={styles.sub}>Review student online applications</ThemedText>
        </View>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.statsRow}>
          <ThemedView style={styles.statCard}>
            <Inbox size={18} color="#fbbf24" style={{ marginBottom: 4 }} />
            <ThemedText style={styles.statNum}>{appsList.length}</ThemedText>
        ...
        {isLoading ? (
          ...
          <ThemedView style={styles.emptyCard}>
            ...
            <ThemedText style={styles.emptyTitle}>No Admission Applications Submitted</ThemedText>
          </ThemedView>
        ) : (
          <View style={{ gap: 10 }}>
            {appsList.map((item: any, idx: number) => {
              const name = `${item.firstName || ''} ${item.lastName || ''}`.trim() || 'Applicant';
              const appliedClass = item.applyingForClass || 'Class Arm';
              ...
              return (
                <ThemedView key={item._id || item.id || idx} style={styles.itemCard}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <ThemedText style={styles.itemSub}>Applying for: {appliedClass}</ThemedText>
                  </View>
                  <Badge label={item.status || 'PENDING'} variant="warning" size="sm" />
                </ThemedView>
              );
            })}
          </View>
