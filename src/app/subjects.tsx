...
export default function SubjectsScreen() {
  ...
  const { data: subjectsList = [], isLoading, isError, refetch } = useQuery({
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <ThemedText style={styles.sub}>Subject allocation by grade</ThemedText>
        </View>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {isLoading ? (
          ...
          <ThemedView style={styles.emptyCard}>
            <ThemedText style={styles.emptyTitle}>Unable to load subjects</ThemedText>
          </ThemedView>
        ) : subjectsList.length === 0 ? (
          <ThemedView style={styles.emptyCard}>
            <BookOpen size={32} color="#64748b" style={{ marginBottom: 8 }} />
            <ThemedText style={styles.emptyTitle}>No Subjects Registered</ThemedText>
          </ThemedView>
        ) : (
          <View style={{ gap: 10 }}>
            {subjectsList.map((item: any, idx: number) => {
              const code = item.code || `SUB-${idx + 1}`;
              ...
              return (
                <ThemedView key={item._id || item.id || idx} style={styles.itemCard}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <ThemedText style={styles.itemSub}>Code: {code}</ThemedText>
                  </View>
                  <Badge label={item.category || 'General'} variant="info" size="sm" />
                </ThemedView>
              );
            })}
          </View>
