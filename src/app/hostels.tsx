...
import { ArrowLeft, Home as HomeIcon, RefreshCw } from 'lucide-react-native';
...
export default function HostelsScreen() {
  ...
  const { data: hostelsList = [], isLoading, isError, refetch } = useQuery({
    queryKey: ['admin-hostels-list'],
    queryFn: async () => {
      const res = await apiClient.get('/hostels').catch(() => ({ data: [] }));
      const raw = res.data;
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={{ flex: 1 }}>
          <ThemedText style={styles.title}>Hostel Management</ThemedText>
          <ThemedText style={styles.sub}>Room allocations & bed spaces</ThemedText>
        </View>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.statsRow}>
          <ThemedView style={styles.statCard}>
            <HomeIcon size={18} color="#4ade80" style={{ marginBottom: 4 }} />
            <ThemedText style={styles.statNum}>{hostelsList.length}</ThemedText>
            <ThemedText style={styles.statLabel}>Hostel Blocks</ThemedText>
          </ThemedView>
        </View>
        ...
        {isLoading ? (
          ...
          <ThemedView style={styles.emptyCard}>
            <ThemedText style={styles.emptyTitle}>Unable to load hostels</ThemedText>
          </ThemedView>
        ) : hostelsList.length === 0 ? (
          <ThemedView style={styles.emptyCard}>
            <HomeIcon size={32} color="#64748b" style={{ marginBottom: 8 }} />
            <ThemedText style={styles.emptyTitle}>No Hostel Blocks Registered</ThemedText>
          </ThemedView>
        ) : (
          <View style={{ gap: 10 }}>
            {hostelsList.map((item: any, idx: number) => {
              const name = item.name || 'Hostel Block';
              const capacity = item.capacity || 50;

              return (
                <ThemedView key={item._id || item.id || idx} style={styles.itemCard}>
                  <View style={styles.iconBox}>
                    <HomeIcon size={18} color="#4ade80" />
                  </View>
                  <View style={{ flex: 1 }}>
                    <ThemedText style={styles.itemName}>{name}</ThemedText>
                    <ThemedText style={styles.itemSub}>Capacity: {capacity} Students</ThemedText>
                  </View>
...
const styles = StyleSheet.create({
  ...
  iconBox: { width: 38, height: 38, borderRadius: 10, backgroundColor: 'rgba(74, 222, 128, 0.12)', justifyContent: 'center', alignItems: 'center' },
