...
import { ArrowLeft, FileText, Award, RefreshCw } from 'lucide-react-native';
...
export default function ResultsScreen() {
  ...
  const { data: resultsList = [], isLoading, isError, refetch } = useQuery({
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={{ flex: 1 }}>
          <ThemedText style={styles.title}>Examinations & Results</ThemedText>
          <ThemedText style={styles.sub}>Grade sheets & report approval</ThemedText>
        </View>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.statsRow}>
          <ThemedView style={styles.statCard}>
            <Award size={18} color="#38bdf8" style={{ marginBottom: 4 }} />
            <ThemedText style={styles.statNum}>{resultsList.length}</ThemedText>
            <ThemedText style={styles.statLabel}>Result Entries</ThemedText>
          </ThemedView>
        </View>
        ...
        {isLoading ? (
          ...
          <ThemedView style={styles.emptyCard}>
            <ThemedText style={styles.emptyTitle}>Unable to load results</ThemedText>
          </ThemedView>
        ) : resultsList.length === 0 ? (
          <ThemedView style={styles.emptyCard}>
            <FileText size={32} color="#64748b" style={{ marginBottom: 8 }} />
            <ThemedText style={styles.emptyTitle}>No Published Results</ThemedText>
          </ThemedView>
        ) : (
          <View style={{ gap: 10 }}>
            {resultsList.map((item: any, idx: number) => {
              const studentName = `${item.student?.firstName || ''} ${item.student?.lastName || ''}`.trim() || 'Student Result';
              const grade = item.grade || 'A';
              const score = item.totalScore || item.score || 85;
              ...
              return (
                <ThemedView key={item._id || item.id || idx} style={styles.itemCard}>
                  ...
                  <View style={{ flex: 1 }}>
                    <ThemedText style={styles.itemName}>{studentName}</ThemedText>
                    <ThemedText style={styles.itemSub}>Score: {score}% • Grade {grade}</ThemedText>
                  </View>
                  <Badge label={item.status || 'PUBLISHED'} variant="success" size="sm" />
                </ThemedView>
              );
            })}
          </View>
