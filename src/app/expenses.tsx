...
import { ArrowLeft, DollarSign, RefreshCw } from 'lucide-react-native';
...
export default function ExpensesScreen() {
  ...
  const { data: expensesList = [], isLoading, isError, refetch } = useQuery({
    ...
    queryFn: async () => {
      const res = await apiClient.get('/expenses').catch(() => ({ data: [] }));
      const raw = res.data;
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={{ flex: 1 }}>
          <ThemedText style={styles.title}>Expenses & Outflows</ThemedText>
          <ThemedText style={styles.sub}>Operational expenditure logs</ThemedText>
        </View>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.statsRow}>
          <ThemedView style={styles.statCard}>
            <DollarSign size={18} color="#f87171" style={{ marginBottom: 4 }} />
            <ThemedText style={styles.statNum}>{expensesList.length}</ThemedText>
            <ThemedText style={styles.statLabel}>Expense Records</ThemedText>
          </ThemedView>
        </View>
        ...
        {isLoading ? (
          ...
          <ThemedView style={styles.emptyCard}>
            <ThemedText style={styles.emptyTitle}>Unable to load expenses</ThemedText>
          </ThemedView>
        ) : expensesList.length === 0 ? (
          <ThemedView style={styles.emptyCard}>
            <DollarSign size={32} color="#64748b" style={{ marginBottom: 8 }} />
            <ThemedText style={styles.emptyTitle}>No Expense Records Logged</ThemedText>
          </ThemedView>
        ) : (
          <View style={{ gap: 10 }}>
            {expensesList.map((item: any, idx: number) => {
              const title = item.title || item.category || 'School Expense';
              ...
              return (
                <ThemedView key={item._id || item.id || idx} style={styles.itemCard}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <ThemedText style={styles.itemSub}>{item.category || 'General Operations'}</ThemedText>
                  </View>
...
const styles = StyleSheet.create({
  ...
  iconBox: { width: 38, height: 38, borderRadius: 10, backgroundColor: 'rgba(248, 113, 113, 0.12)', justifyContent: 'center', alignItems: 'center' },
  itemName: { fontSize: 15, fontWeight: 'bold', color: '#f8fafc' },
  itemSub: { fontSize: 12, color: '#94a3b8', marginTop: 2 },
  amountText: { fontSize: 15, fontWeight: 'bold', color: '#f87171' },
