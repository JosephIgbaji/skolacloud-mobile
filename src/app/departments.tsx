...
import { ArrowLeft, Briefcase, RefreshCw } from 'lucide-react-native';
...
export default function DepartmentsScreen() {
  ...
  const { data: deptList = [], isLoading, isError, refetch } = useQuery({
    queryKey: ['admin-departments-list'],
    queryFn: async () => {
      const res = await apiClient.get('/departments').catch(() => ({ data: [] }));
      const raw = res.data;
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={{ flex: 1 }}>
          <ThemedText style={styles.title}>Academic Departments</ThemedText>
          <ThemedText style={styles.sub}>Faculty & administrative arms</ThemedText>
        </View>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.statsRow}>
          <ThemedView style={styles.statCard}>
            <Briefcase size={18} color="#fb7185" style={{ marginBottom: 4 }} />
            <ThemedText style={styles.statNum}>{deptList.length}</ThemedText>
        ...
        {isLoading ? (
          ...
          <ThemedView style={styles.emptyCard}>
            <ThemedText style={styles.emptyTitle}>Unable to load departments</ThemedText>
          </ThemedView>
        ) : deptList.length === 0 ? (
          <ThemedView style={styles.emptyCard}>
            <Briefcase size={32} color="#64748b" style={{ marginBottom: 8 }} />
            <ThemedText style={styles.emptyTitle}>No Departments Configured</ThemedText>
          </ThemedView>
        ) : (
          <View style={{ gap: 10 }}>
            {deptList.map((item: any, idx: number) => {
              const name = item.name || 'Department';
              const hod = item.headOfDepartment?.name || item.hod || 'Unassigned';

              return (
                <ThemedView key={item._id || item.id || idx} style={styles.itemCard}>
                  <View style={styles.iconBox}>
                    <Briefcase size={18} color="#fb7185" />
                  </View>
                  <View style={{ flex: 1 }}>
                    <ThemedText style={styles.itemName}>{name}</ThemedText>
                    <ThemedText style={styles.itemSub}>Head: {hod}</ThemedText>
                  </View>
...
const styles = StyleSheet.create({
  ...
  iconBox: { width: 38, height: 38, borderRadius: 10, backgroundColor: 'rgba(251, 113, 133, 0.12)', justifyContent: 'center', alignItems: 'center' },
