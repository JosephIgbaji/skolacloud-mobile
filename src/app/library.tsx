...
export default function LibraryScreen() {
  ...
  const { data: booksList = [], isLoading, isError, refetch } = useQuery({
    queryKey: ['admin-library-list'],
    queryFn: async () => {
      const res = await apiClient.get('/library/books').catch(() => ({ data: [] }));
      const raw = res.data;
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={{ flex: 1 }}>
          <ThemedText style={styles.title}>Library Management</ThemedText>
          <ThemedText style={styles.sub}>Book catalog & loan tracking</ThemedText>
        </View>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.statsRow}>
          <ThemedView style={styles.statCard}>
            <BookOpen size={18} color="#fbbf24" style={{ marginBottom: 4 }} />
            <ThemedText style={styles.statNum}>{booksList.length}</ThemedText>
            <ThemedText style={styles.statLabel}>Books Cataloged</ThemedText>
          </ThemedView>
        </View>
        ...
        {isLoading ? (
          ...
          <ThemedView style={styles.emptyCard}>
            <ThemedText style={styles.emptyTitle}>Unable to load library catalog</ThemedText>
          </ThemedView>
        ) : booksList.length === 0 ? (
          <ThemedView style={styles.emptyCard}>
            <BookOpen size={32} color="#64748b" style={{ marginBottom: 8 }} />
            <ThemedText style={styles.emptyTitle}>No Books Cataloged</ThemedText>
          </ThemedView>
        ) : (
          <View style={{ gap: 10 }}>
            {booksList.map((item: any, idx: number) => {
              const title = item.title || 'Library Book';
              ...
              return (
                <ThemedView key={item._id || item.id || idx} style={styles.itemCard}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <ThemedText style={styles.itemSub}>Author: {author}</ThemedText>
                  </View>
...
const styles = StyleSheet.create({
  ...
  iconBox: { width: 38, height: 38, borderRadius: 10, backgroundColor: 'rgba(251, 191, 36, 0.12)', justifyContent: 'center', alignItems: 'center' },
